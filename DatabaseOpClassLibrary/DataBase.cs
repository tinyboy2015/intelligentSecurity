...
namespace DatabaseOpClassLibrary
{
    public class DataBaseSQLserver
    {
        ...
        protected void open()
        {
            ...
            {
                SqlCn = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["QSFB"].ConnectionString); //ConfigurationSettings.AppSettings.Get
        ...
        public SqlCommand CreateCommand(string strSql, int iFlag)
        {
            open();
        ...
        /// <param name="iFlag">1：存储过程；0：SQL语句</param>
        /// <returns>SqlCommand</returns>
        public SqlCommand CreateCommand(string strSql, SqlParameter[] Params, int iFlag)
        {
            open();
        ...
        /// <param name="Params">参数集</param>
        /// <returns></returns>
        public SqlDataReader GetReader(string strsql, SqlParameter[] Params)
        {
            OpenDB();
        ...
        /// <param name="iFlag">1：存储过程；0：SQL语句</param>
        /// <returns></returns>
        public SqlDataReader GetReader(string strsql, SqlParameter[] Params, int iFlag)
        {
            OpenDB();
        ...
        /// <param name="Value">Param value.</param>
        /// <returns>New parameter.</returns>
        public SqlParameter MakeParam(string ParamName, SqlDbType DbType, System.Int32 Size, ParameterDirection Direction, object Value)
        {
            SqlParameter param;
        ...
        public int RunProcRtnInt(string sqlCmd, SqlParameter[] dbParams)
        {
            try
            {
                int iReturn = 0;
        ...
        /// <returns>影响的行数</returns>
        public int RunProcRtnInt(string procName, SqlParameter[] dbParams, int iFlag)
        {
            try
            {
                int iReturn = 0;

                SqlCommand cmd = CreateCommand(procName, iFlag);
        ...
        public object RunProcObject(string procName, SqlParameter[] prams, int iFlag)
        {
            try
            {
                object objResult;

                SqlCommand cmd = CreateCommand(procName, prams, iFlag);
        ...
        public void RunProcRtnDataRow(string procName, SqlParameter[] prams, int iFlag, out DataRow dataRow)
        {
            SqlCommand cmd = CreateCommand(procName, prams, iFlag);
        ...
        private SqlParameter[] FormatParams(System.Collections.Generic.Dictionary<string, string> Params, System.Collections.Generic.Dictionary<string, string> ParamsType)
        {
            SqlParameter[] parameters = new SqlParameter[Params.Count];
            int i = 0;
            foreach (System.Collections.Generic.KeyValuePair<string, string> member in Params)
            {
                ...
                parameters[i].SqlDbType = ParamsType != null && ParamsType.ContainsKey(member.Key) ? ConvertType(ParamsType[member.Key]) : SqlDbType.NVarChar;
                i++;
            }
            return parameters;
        }
        ...
        private SqlDbType ConvertType(string type)
        {
            switch (type)
            {
